<?php
session_start();
require(__DIR__ . "/MQPublish.inc.php");
require(__DIR__ . "/header.php");
if (isset($_SESSION["user"])) {
    $user = $_SESSION["user"]["user_name"];
    if (isset($_POST["submit"])) {
        ob_start();
        $preference = ((array)tracking($_POST["family"], $user))["message"];
        ob_end_clean();
    }
    ob_start();
    $info = (array)getTrackingInfo($user);
    ob_end_clean();
} else {
    echo "<script>alert('You must be logged in to access this page.')</script>";
    echo "<script>window.location = 'login.php'; </script>";
}
ob_start();
$data = getCache()->data;
ob_end_clean();
?>

<!DOCTYPE html>
<html lang="en">

<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1, shrink-to-fit=no">
    <link rel="stylesheet" href="CSS/style.css">
    <link rel="stylesheet" href="https://maxcdn.bootstrapcdn.com/bootstrap/4.0.0/css/bootstrap.min.css" integrity="sha384-Gn5384xqQ1aoWXA+058RXPxPg6fy4IWvTNh0E263XmFcJlSAwiGgFAW/dAiS6JXm" crossorigin="anonymous">
    <script src="https://code.jquery.com/jquery-3.2.1.slim.min.js" integrity="sha384-KJ3o2DKtIkvYIK3UENzmM7KCkRr/rE9/Qpg6aAZGJwFDMVNA/GpGFF93hXpG5KkN" crossorigin="anonymous"></script>
    <script src="https://cdnjs.cloudflare.com/ajax/libs/popper.js/1.12.9/umd/popper.min.js" integrity="sha384-ApNbgh9B+Y1QKtv3Rn7W3mgPxhU9K/ScQsAP7hUibX39j7fakFPskvXusvfa0b4Q" crossorigin="anonymous"></script>
    <script src="https://maxcdn.bootstrapcdn.com/bootstrap/4.0.0/js/bootstrap.min.js" integrity="sha384-JZR6Spejh4U02d8jOt6vLEHfe/JQGiRRSQQxSfFWpi1MquVdAyjUar5+76PVCmYl" crossorigin="anonymous"></script>
    <script src="JS/desclimit.js"></script> <!-- Add 'limitdesc' as a class variable to limit characters -->
    <link rel="icon" href="IMG/490IconPic.png">
    <title>Popularity Rankings</title>
</head>

<body>


    <div class="container d-flex justify-content-center mt-50 mb-50">
        <div class="card" style="width: 700px;">
            <div class="card" style="width: 700px;">
                <div class="card-body">
                    <h5 class="card-title text-center mb-0">Statistics</h5>
                </div>
            </div>
            <div class="card" style="width: 700px;">
                <ul class="list-group list-group-flush">
                    <div class="card-body">
                        <h5 class="card-title text-center mb-0">Popularity Ratings</h5>
                    </div>


                    <?php
                    ob_start();
                    $countData = $info["countData"];
                    ob_end_clean();
                    foreach ($countData as $key => $value) {
                        //product family : count of trackers
                        $arr[((array)$value)["product_family_name"]] = ((array)$value)["count"];
                    }
                    arsort($arr, SORT_NUMERIC);
                    $count = 1;
                    foreach ($arr as $key => $value) {
                        //product family : count of trackers
                        echo "<li class='list-group-item first-place'> #" . $count . " Most Tracked: " . $key . "</li><br>";
                        $count++;
                    }

                    ?>
                </ul>

            </div>
        </div>
    </div>